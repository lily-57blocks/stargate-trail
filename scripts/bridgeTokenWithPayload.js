...
async function setReceiver(tokenBridger) {
  const receiverAddress = getContractAddresses("polygonMumbai", "Receiver")[
    "Receiver"
  ];
  const tx = await tokenBridger.updateDstContract(receiverAddress);
  console.log("updateDstContract tx: ", tx.hash);
  await tx.wait();
}

async function bridgeTokenWithPayload(amount) {
  const network = hre.network.name;
  if (network !== "goerli") {
    console.log("Skipping bridgeTokenWithPayload on non-goerli network");
    return;
  }
  const destChain = "polygonMumbai";
  console.log(`bridge token from ${network} to ${destChain}`);

  const contractName = "USDCBridgeTokenHelperWithPayload";

  const usdcBridgeTokenHelperWithPayloadAddress = getContractAddresses(
    network,
    contractName
  )[contractName];
  console.log(
    `usdcBridgeTokenHelperWithPayloadAddress: ${usdcBridgeTokenHelperWithPayloadAddress}`
  );
  ...
  await approve(sender, usdcBridgeTokenHelperWithPayloadAddress, amount);

  let usdcBridgeTokenHelper = await hre.ethers.getContractAt(
    contractName,
    usdcBridgeTokenHelperWithPayloadAddress
  );

  const dstContract = await usdcBridgeTokenHelper.dstContract();
  if (dstContract == ethers.ZeroAddress) {
    await setReceiver(usdcBridgeTokenHelper);
  }
  ...
  let tx = await usdcBridgeTokenHelper
    .connect(sender)
    .bridgeTokenAndExecute(chainId, 1, amount, receiver, { value: relayerFee });
  console.log(`bridgeTokenAndExecute tx: ${tx.hash}`);
}

bridgeTokenWithPayload("1").catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
